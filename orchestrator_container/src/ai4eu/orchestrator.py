...
class Core:
    ...
    def merge_and_compile_protobuf_files(self, protofiles: Dict[str, str], output_filename='all_in_one.proto') -> None:
        ...
        # Save the descriptor to a temp file
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pb") as tmp_f:
            tmp_f.write(merged_fds.SerializeToString())
            descriptor_path = tmp_f.name
        ...
        # Run protoc
        subprocess.run(command, shell=True, stderr=sys.stderr, check=True)
